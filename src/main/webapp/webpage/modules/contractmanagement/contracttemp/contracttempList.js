	...
	$('#table').bootstrapTable({
		  //请求方法
                   ...
                   if($el.data("item") == "edit"){
                   	edit(row.id);
                        ...
                        jp.confirm('确认要删除该记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/contracttemp/delete?id="+row.id, function(data){
                   	  		if(data.success){
			...
			,{
				...
				sortable: true,
		        formatter:function(value, row , index){
		        	return jp.getDictLabel(${fns:toJson(fns:getDictList('contract_temp_type'))}, value, "-");
		        }
			}
			,{
		        field: 'remarks',
