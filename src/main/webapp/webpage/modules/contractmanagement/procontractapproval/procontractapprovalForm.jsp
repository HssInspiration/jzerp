	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$("#test_data").bsSuggest({
				  ...
				  keyField : "word",
				  getDataMethod : "url",
				  url : "${ctx}/procontractapproval/getProContractList?contractName=",
				  processData : function(json) {
				      ...
				      len = json.length;
				      for (i = 0; i < len; i++) {
				    	  console.log(json[i].id+"+"+json[i].contractName);
				          data.value.push({
...
// 				$.ajax({
// 					url:"${ctx}/procontractapproval/getprocontractapprovalByProgramId",
// 					data:jsonData,
// 					type:"post",
...
// 			});
			...
			validateForm = $("#inputForm").validate({
				submitHandler: function(form){
					...
					jp.post("${ctx}/procontractapproval/save",$('#inputForm').serialize(),function(data){
						if(data.success){
...
<body class="bg-white">
		<form:form id="inputForm" modelAttribute="proContractApproval" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<table class="table table-bordered">
		   <tbody>
				<tr>
					...
					<td class="width-35">
						<input type="hidden" class="form-control" name= "proContract.id" id = "proContractId" value = "${proContractApproval.proContract.id}">
						<c:if test="${isAdd}">
							<div class="row">
				                <div class="col-lg-2">
				                    <div class="input-group">
				                        <input type="text" class="form-control" <c:if test="${edit}" >readOnly="true"</c:if>  id="test_data" value = "${procontractapproval.proContract.contractName}">
			            ...
			            <c:if test="${edit }">
			            	<input type="text" class="form-control" readOnly="true" value = "${proContractApproval.proContract.contractName}">
			            </c:if>		
					</td>
				</tr>
				<tr>
