                   ...
                   if($el.data("item") == "edit"){
                   	edit(row.id);
                        ...
                        jp.confirm('确认要删除该保证金信息管理记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/deposit/delete?id="+row.id, function(data){
                   	  		if(data.success){
			...
			,{
		        ...
		        ,formatter:function(value, row , index){
		        	...
		        	if(status == "0" || status == "3"){
		        		return '<a  href="#" onclick="jp.openDialog(\'编辑保证金信息\', \'${ctx}/deposit/form?id='+row.id+'\',\'1000px\', \'600px\')">'+value+'</a>';
		        	}else{
...
//		        	return jp.getDictLabel(${fns:toJson(fns:getDictList('deposit_checkstatus'))}, value, "-");
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/deposit/import/template';
				  },
  ...
  function getIdSelections() {
      return $.map($("#table").bootstrapTable('getSelections'), function (row) {
    	  console.log("000000:"+row.checkStatus)
          return row.id
      });
  }
...
//	   </shiro:hasPermission>
//	  <shiro:lacksPermission name="deposit:deposit:edit">
