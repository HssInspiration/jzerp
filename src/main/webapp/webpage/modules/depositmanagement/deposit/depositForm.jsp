							...
							if(data.length>0){
								for(var i=0;i<=data.length-1;i++){
									if(data[i]==value){
										...
										//设置$("#depositName").val(name);
										var itemLabel = $('select option:selected').text()
										console.log("itemLabel:"+itemLabel);
										var name = $("#depositName").val();
										$("#depositName").val(name+itemLabel);
										break;
									}
								}
...
// 					async:false,
			...
			$("#depositType").bind("change",function(){//选择改变后先做验证，是否已经登记
				...
				if(programId != undefined || proId != null ){
					//发送验证：开始
		...
		<table class="table table-bordered">
		   <tbody>
				<tr>
					...
					<td class="width-35">
						<form:input path="depositName" readOnly="true"  htmlEscape="false" class="form-control required"/>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td class="width-35">
							<form:select path="depositType"  class="form-control required">
								...
								<form:options id="itemLabel" items="${fns:getDictList('deposit_type')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
							</form:select>
