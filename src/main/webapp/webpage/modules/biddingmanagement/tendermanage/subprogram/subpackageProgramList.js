...
$(document).ready(function() {
	$('#table').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/tendermanage/subpackageProgram/data",
                   ...
                   if($el.data("item") == "edit"){
                   	edit(row.id);
                        ...
                        jp.confirm('确认要删除该子项目工程管理记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/tendermanage/subpackageProgram/delete?id="+row.id, function(data){
                   	  		if(data.success){
...
//							'</a> ',
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/tendermanage/subpackageProgram/import/template';
				  },
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该子项目工程管理记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/tendermanage/subpackageProgram/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#table').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
   ...
   function add(){
	  jp.openDialog('新增子项目工程管理', "${ctx}/tendermanage/subpackageProgram/form",'800px', '500px', $('#table'));
  }
