...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			var getUrl,actSubContractId=$("#id").val();
			console.log("getUrl1:"+getUrl);
			console.log("actSubContractId1:"+actSubContractId);
			if(actSubContractId!=null){
				getUrl = "${ctx}/oa/actSubContract/getSubProContractByName?subProContractName=";
			}else{
				getUrl = "${ctx}/oa/actSubContract/getAppointSubProContractByName?subProContractName=";
			}
			...
			$("#test_data").on('onSetSelectValue', function (e, keyword) {//下拉框值选中时调用
				...
				//2.获取完id传入后台获取对应合同信息;
				$.ajax({
					url:"${ctx}/oa/actSubContract/getContractTextByContractId",
					data:jsonData,
					type:"post",
...
<!-- 							<div class="col-sm-10"> -->
<%-- 								<input type="hidden" class="form-control " name= "subProContract.id"  value = "${actSubContract.subProContract.id}"> --%>
<%-- 								<form:input path="subProContract.subProContractName" readOnly="true" class="form-control required"/> --%>
<!-- 							</div> -->
<%-- 						</c:if> --%>
					</div>
					<div class="form-group">
					...
					<c:if test="${actSubContract.act.taskDefKey eq 'modify'}">
						<div class="form-group">
...
<!-- 					<div class="form-group"> -->
<!-- 						<label class="col-sm-2 control-label">设定审批人员：</label> -->
<!-- 						<div class="col-sm-10" > -->
<%-- 							<sys:userselect id="user" name="userId" value="${actSubContract.user.id}" labelName="user.name" labelValue="${actSubContract.user.name}" --%>
<%--  								 isMultiSelected="true" cssClass="form-control required"/>   --%>
<!-- 						</div> -->
<!-- 					</div> -->
					<div class="form-group">
						...
						<div class="form-group text-center">
							...
							<div>
								...
								<c:if test="${not empty actSubContract.id}">
									...
									<input id="actSubContractId" type="hidden" value="${actSubContract.id}"/>
								</c:if>
							</div>
						</div>
					</div>
